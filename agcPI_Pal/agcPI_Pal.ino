 ...
 * 
 * This palette enabled version uses a PI (proportional integral) control loop.
 * 
...
void ledShow() {
  ...
  leds[(millis() % (NUM_LEDS-1)) +1] = ColorFromPalette(currentPalette, samplePI, samplePI, currentBlending);
...
void agcPI() {                                                // A PI Control loop to automatically adjust sound sensitivity.
  ...
  err = targetAgc - samplePIAvg;                              // Calculate the average error from the target. Is continuously going up.
  ...
  if (sensitivity <= 3000) sensitivity = 3000;                // Minimum sensitity multiplier should be 1. Andrew Tuline.
  samplePI = abs(sample * sensitivity / 3000);
  if(samplePI > 255) samplePI = 255;

  samplePIAvg = ((samplePIAvg * 15) + samplePI) / 16;         // Smooth it out over the last 16 samples and use as feedback for the PI loop.

//------------ Oscilloscope output ---------------------------
//  Serial.print(sample); Serial.print(" ");
//  Serial.print(sampleAvg); Serial.print(" ");
//  Serial.proint(micLev); Serial.print(" ");
